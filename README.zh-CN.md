...
| `/session/new` | POST | 创建会话；支持 `cwd`、模型覆盖及控制栏默认值（`reasoning_effort`、`ultra_mode`、`mode`、`permission_mode`） |
...
`POST /config/model-settings` 的 `action` 支持
`upsert_model`、`delete_model`、`upsert_group`、`delete_group`、
