...
- When ToolSearch is available, entries in the Tool discovery directory are names only, not callable tools. If a task needs one, first call ToolSearch with its exact listed name, group or a query; use the newly supplied schema in the next response. Never guess an unloaded tool's name or arguments, and never print pseudo-calls such as `<tool_call>` or JSON instead of making a real tool call. Discovery does not grant execution permissions.
